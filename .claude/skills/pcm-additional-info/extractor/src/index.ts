...
import { fetchConfluencePage, sleep } from "./fetcher.js";
import { parseAdditionalInfoTables } from "./parser.js";
import type { OutputData, PageConfig } from "./types.js";
...
async function main(): Promise<void> {
  ...
  for (let i = 0; i < config.pages.length; i++) {
    ...
    console.log(`[${i + 1}/${config.pages.length}] Fetching: ${page.title} (pageId=${page.pageId})`);

    try {
      const { html, url } = await fetchConfluencePage(page.pageId, baseUrl, config.retryDelaysMs);
